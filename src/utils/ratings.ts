...
export const getRating = (metric: keyof MetricScores, value: number, gender: 'male' | 'female' = 'male'): { text: string, color: string } => {
    if (gender === 'female') {
        const fRatings: Record<string, { text: string, color: string }> = {
            ...
            fWHR: value >= 1.55 ? { text: 'perfect (heart/oval)', color: 'text-green-400' } : value >= 1.45 ? { text: 'good', color: 'text-blue-400' } : { text: 'narrow face', color: 'text-orange-400' },
            ...
            bigonialRatio: value >= 0.72 && value <= 0.80 ? { text: 'perfect oval jaw', color: 'text-green-400' } : value >= 0.68 ? { text: 'good', color: 'text-blue-400' } : value < 0.65 ? { text: 'very wide jaw', color: 'text-orange-400' } : { text: 'narrow', color: 'text-orange-400' },
            ...
            pfl: value >= 2.8 && value <= 3.5 ? { text: 'perfect doe eyes', color: 'text-green-400' } : value > 3.5 ? { text: 'long fissures', color: 'text-blue-400' } : { text: 'round eyes', color: 'text-yellow-400' },
            esr: value >= 0.45 && value <= 0.47 ? { text: 'perfect', color: 'text-green-400' } : value >= 0.42 && value <= 0.49 ? { text: 'good', color: 'text-blue-400' } : { text: 'suboptimal spacing', color: 'text-yellow-400' },
            ...
            facialFifthsRatio: value >= 0.95 && value <= 1.05 ? { text: 'ideal facial fifths', color: 'text-green-400' } : value >= 0.90 && value <= 1.10 ? { text: 'good', color: 'text-blue-400' } : { text: 'imbalanced fifths', color: 'text-orange-400' },
            cervicomentalAngle: value >= 105 && value <= 120 ? { text: 'ideal neck angle', color: 'text-green-400' } : value >= 100 && value <= 125 ? { text: 'good', color: 'text-blue-400' } : value < 100 ? { text: 'soft neck angle', color: 'text-orange-400' } : { text: 'over-open angle', color: 'text-yellow-400' },
            overallSymmetry: value >= 93 ? { text: 'perfectly symmetric', color: 'text-green-400' } : value >= 85 ? { text: 'very symmetric', color: 'text-blue-400' } : value >= 80 ? { text: 'acceptable symmetry', color: 'text-yellow-400' } : { text: 'asymmetric', color: 'text-orange-400' },
            ipd: value >= 0.45 && value <= 0.47 ? { text: 'ideal eye spacing', color: 'text-green-400' } : value >= 0.42 && value <= 0.49 ? { text: 'good', color: 'text-blue-400' } : { text: 'suboptimal spacing', color: 'text-yellow-400' },
            ...
            noseWidthRatio: value >= 0.18 && value <= 0.23 ? { text: 'ideal narrow nose', color: 'text-green-400' } : value > 0.28 ? { text: 'wide nose', color: 'text-orange-400' } : value < 0.16 ? { text: 'very narrow', color: 'text-yellow-400' } : { text: 'acceptable', color: 'text-blue-400' },
            ...
            collagenIndex: value > 90 ? { text: 'glass skin', color: 'text-green-400' } : value > 75 ? { text: 'clear skin', color: 'text-blue-400' } : value > 50 ? { text: 'textured', color: 'text-yellow-400' } : { text: 'acne / heavy texture', color: 'text-red-400' },
            ...
            uee: value < 0.25 ? { text: 'hooded / ideal', color: 'text-green-400' } : value < 0.35 ? { text: 'good', color: 'text-blue-400' } : value > 0.45 ? { text: 'excessive (bug eyes)', color: 'text-red-400' } : { text: 'noticeable UEE', color: 'text-yellow-400' },
    ...
    const mRatings: Record<string, { text: string, color: string }> = {
        canthalTilt: value >= 4 && value <= 6 ? { text: 'ideal canthal tilt', color: 'text-green-400' } : value > 2 ? { text: 'good', color: 'text-blue-400' } : value < 0 ? { text: 'negative canthal tilt', color: 'text-red-400' } : { text: 'neutral', color: 'text-yellow-400' },
        fWHR: value >= 1.80 ? { text: 'ideal broad face', color: 'text-green-400' } : value >= 1.70 ? { text: 'good', color: 'text-blue-400' } : { text: 'narrow face', color: 'text-orange-400' },
        midfaceRatio: value >= 1.00 && value <= 1.10 ? { text: 'ideal compact midface', color: 'text-green-400' } : (value >= 0.95 && value <= 1.15) ? { text: 'good', color: 'text-blue-400' } : value < 0.95 ? { text: 'excessively compact', color: 'text-red-400' } : { text: 'long midface', color: 'text-red-400' },
        gonialAngle: value >= 115 && value <= 130 ? { text: 'ideal jaw angle', color: 'text-green-400' } : value >= 105 && value <= 135 ? { text: 'good', color: 'text-blue-400' } : { text: 'soft jawline', color: 'text-orange-400' },
        chinToPhiltrumRatio: value >= 2.0 && value <= 2.25 ? { text: 'ideal', color: 'text-green-400' } : value < 1.6 ? { text: 'long philtrum / weak chin', color: 'text-red-400' } : value > 3.0 ? { text: 'very long chin', color: 'text-orange-400' } : value < 1.8 ? { text: 'slightly long philtrum', color: 'text-yellow-400' } : { text: 'acceptable', color: 'text-blue-400' },
        mouthToNoseWidthRatio: value >= 1.50 && value <= 1.62 ? { text: 'ideal', color: 'text-green-400' } : value >= 1.40 ? { text: 'good', color: 'text-blue-400' } : { text: 'narrow mouth', color: 'text-orange-400' },
        bigonialRatio: value >= 0.72 && value <= 0.78 ? { text: 'ideal jaw width', color: 'text-green-400' } : value >= 0.68 && value <= 0.82 ? { text: 'good', color: 'text-blue-400' } : value > 0.82 ? { text: 'very narrow jaw', color: 'text-orange-400' } : { text: 'very wide jaw', color: 'text-orange-400' },
        ...
        pfl: value >= 3.0 && value <= 3.5 ? { text: 'perfect horizontal eyes', color: 'text-green-400' } : value > 3.5 ? { text: 'good', color: 'text-blue-400' } : { text: 'slightly too round eyes', color: 'text-yellow-400' },
        esr: value >= 0.45 && value <= 0.47 ? { text: 'perfect', color: 'text-green-400' } : value >= 0.42 && value <= 0.49 ? { text: 'good', color: 'text-blue-400' } : { text: 'suboptimal spacing', color: 'text-yellow-400' },
        ...
        foreheadHeightRatio: value >= 0.30 && value <= 0.35 ? { text: 'ideal forehead', color: 'text-green-400' } : value > 0.38 ? { text: 'fivehead (large)', color: 'text-orange-400' } : value < 0.28 ? { text: 'short forehead', color: 'text-yellow-400' } : { text: 'acceptable', color: 'text-blue-400' },
        noseWidthRatio: value >= 0.18 && value <= 0.22 ? { text: 'ideal nose width', color: 'text-green-400' } : value > 0.26 ? { text: 'wide nose', color: 'text-orange-400' } : value < 0.16 ? { text: 'narrow nose', color: 'text-yellow-400' } : { text: 'acceptable', color: 'text-blue-400' },
        ...
        collagenIndex: value > 90 ? { text: 'glass skin', color: 'text-green-400' } : value > 75 ? { text: 'clear skin', color: 'text-blue-400' } : value > 50 ? { text: 'textured', color: 'text-yellow-400' } : { text: 'acne / heavy texture', color: 'text-red-400' },
        hairQualityScore: value >= 85 ? { text: 'elite grooming', color: 'text-green-400' } : value >= 65 ? { text: 'well-groomed', color: 'text-blue-400' } : value >= 40 ? { text: 'average grooming', color: 'text-yellow-400' } : { text: 'unkempt / poor', color: 'text-red-400' },
        uee: value < 0.25 ? { text: 'hunter eyes (no UEE)', color: 'text-green-400' } : value < 0.35 ? { text: 'good', color: 'text-blue-400' } : value > 0.45 ? { text: 'bug eyes (high UEE)', color: 'text-red-400' } : { text: 'noticeable UEE', color: 'text-yellow-400' },
        philtrumLength: value < 0.08 ? { text: 'compact / ideal', color: 'text-green-400' } : value < 0.10 ? { text: 'good', color: 'text-blue-400' } : value > 0.12 ? { text: 'long (chimp)', color: 'text-red-400' } : { text: 'average', color: 'text-yellow-400' }
    };
...
export const getIdealRange = (metric: keyof MetricScores, gender: 'male' | 'female' = 'male'): string => {
    if (gender === 'female') {
        const fIdeals: Record<string, string> = {
            canthalTilt: '5 to 9.5 deg',
            fWHR: '> 1.55',
            midfaceRatio: '0.8 to 1.05',
            gonialAngle: '105 to 130 deg',
            chinToPhiltrumRatio: '2.0 to 2.75',
            mouthToNoseWidthRatio: '1.45 to 1.6',
            bigonialRatio: '0.68 to 0.80',
            lowerThirdRatio: '0.58 to 0.65',
            pfl: '2.8 to 3.5',
            esr: '0.45 to 0.47',
            eyeToMouthAngle: '47 to 50 deg',
            lipRatio: '1.15 to 1.70',
            facialFifthsRatio: '0.95 to 1.05',
            cervicomentalAngle: '105 to 120 deg',
            overallSymmetry: '93-100 (symmetry score)',
            ipd: '0.45 to 0.47',
            facialThirdsRatio: '95-100 (balance score)',
            foreheadHeightRatio: '0.30 to 0.35',
            noseWidthRatio: '0.18 to 0.23',
            ...
            collagenIndex: '75-100 (clear)',
            hairQualityScore: '65-100 (groomed)',
            uee: '< 0.25 (minimal)',
            philtrumLength: '< 0.10 (compact)'
        };
        return fIdeals[metric] || '';
    }

    const mIdeals: Partial<Record<keyof MetricScores, string>> = {
        canthalTilt: '4 to 6 deg',
        fWHR: '1.8+',
        midfaceRatio: '1.0 to 1.1',
        gonialAngle: '110 to 125 deg',
        chinToPhiltrumRatio: '2.0 to 2.25',
        mouthToNoseWidthRatio: '1.50 to 1.62',
        bigonialRatio: '0.72 to 0.80',
        lowerThirdRatio: '0.62+',
        pfl: '3.0 to 3.5',
        esr: '0.45 to 0.47',
        eyeToMouthAngle: '46 to 50 deg',
