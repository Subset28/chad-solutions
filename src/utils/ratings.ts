...
export const getRating = (metric: keyof MetricScores, value: number, gender: 'male' | 'female' = 'male'): { text: string, color: string } => {
    if (gender === 'female') {
        const fRatings: Record<string, { text: string, color: string }> = {
            ...
            bigonialRatio: value >= 1.15 && value <= 1.3 ? { text: 'perfect oval jaw', color: 'text-green-400' } : value >= 1.1 ? { text: 'good', color: 'text-blue-400' } : value > 1.35 ? { text: 'very wide jaw', color: 'text-orange-400' } : { text: 'narrow', color: 'text-orange-400' },
    ...
    const mRatings: Record<string, { text: string, color: string }> = {
        ...
        fWHR: value >= 1.65 ? { text: 'perfect broad face', color: 'text-green-400' } : value >= 1.55 ? { text: 'good', color: 'text-blue-400' } : { text: 'narrow face', color: 'text-orange-400' },
        ...
        bigonialRatio: value >= 1.05 && value <= 1.25 ? { text: 'perfect wide jaw', color: 'text-green-400' } : value <= 1.35 ? { text: 'good', color: 'text-blue-400' } : { text: 'narrow jaw', color: 'text-orange-400' },
