        ...
        try {
            ...
            if (navigator.canShare?.({ files: [file] })) {
                ...
                await navigator.share({
                    title: `PSL facial analysis: ${pslScore.toFixed(2)} - ${tier}`,
                    text: `Forum-derived PSL facial analysis on Chad Solutions.\nPSL: ${pslScore.toFixed(2)} (${tier})\nchadsolutions.app`,
                    files: [file],
                });
                return;
            }
            ...
            downloadBlob(blob, `chadsolutions-psl-${pslScore.toFixed(2)}.png`);
        } catch (err) {
                    ...
                    <div className="flex-1 flex flex-col gap-6">
                        <div className="relative aspect-square w-full bg-zinc-900 border border-zinc-800 overflow-hidden group">
                            ...
                            <div className="absolute top-4 right-4 bg-black/80 backdrop-blur-md px-4 py-2 border border-white/10">
                                ...
                                <div className="text-[8px] text-zinc-500 text-center -mt-1 tracking-widest">PSL SCORE</div>
                            </div>
                        </div>
                                        ...
                                        <div key={m.key} className="space-y-1">
                                            <div className="flex justify-between text-[8px] text-zinc-500 tracking-widest">
                                                ...
                                                <span className="text-white">
                                                    {typeof val === 'number'
                                                        ? (m.key.includes('Angle') || m.key.includes('Tilt') ? `${val.toFixed(1)} deg` : val.toFixed(2))
                                                        : 'N/A'}
                                                </span>
                                            </div>
