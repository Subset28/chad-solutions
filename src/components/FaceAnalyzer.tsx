...
export default function FaceAnalyzer() {
    ...
    const getRating = (metric: keyof MetricScores, value: number, gender: 'male' | 'female' = 'male'): { text: string, color: string } => {
        ...
        const mRatings: Record<string, { text: string, color: string }> = {
            ...
            midfaceRatio: value >= 0.85 && value <= 1.05 ? { text: 'perfect compact midface', color: 'text-green-400' } : value <= 1.10 ? { text: 'good', color: 'text-blue-400' } : { text: 'significantly too long midface', color: 'text-red-400' },
            ...
            mouthToNoseWidthRatio: value >= 1.45 && value <= 1.62 ? { text: 'perfect', color: 'text-green-400' } : value >= 1.35 ? { text: 'good', color: 'text-blue-400' } : { text: 'narrow mouth', color: 'text-orange-400' },
    ...
    const getIdealRange = (metric: keyof MetricScores, gender: 'male' | 'female' = 'male'): string => {
        ...
        const mIdeals: Record<keyof MetricScores, string> = {
            canthalTilt: '4° to 6°',
            fwfhRatio: '> 1.65',
            midfaceRatio: '0.85 to 1.05',
            gonialAngle: '110° to 125°',
            chinToPhiltrumRatio: '2.0 to 2.25',
            mouthToNoseWidthRatio: '1.45 to 1.62',
